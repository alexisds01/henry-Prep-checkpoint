...
function stringMasLarga(strings) {
  ...
  strings.forEach(function (elemento) {
    ...
    if (elemento.length > stringLargo.length) {
      stringLargo = elemento;
    }
  });
...
function crearClasePersona() {
  ...
  class Persona {
    ...
    getFriends() {
      ...
      var nombresDeAmigos = new Array();

      this.amigos.map(function (elemento) {
        nombresDeAmigos.push(elemento.nombre);
      });
    ...
    getPromedioEdad() {
      ...
      var edadTotal = this.amigos.reduce(function (acum, elemento) {

        return acum + elemento.edad;
      }, 0);

      var promedio = edadTotal / this.amigos.length;
